...
def apply_feature_engineering(df_in: pd.DataFrame, sector: str):
    """Rigorous feature engineering with absolute type enforcement."""
    # Extract raw values to a dict to avoid Series/Object ambiguity
    ...
    # Pre-define Categoricals to keep them strictly as Strings
    CAT_COLS = {
    ...
    if sector == 'aviation':
        ...
        engineered['is_business_travel'] = 1.0 if str(get_val('Type of Travel')).strip() == 'Business travel' else 0.0
        engineered['is_disloyal_customer'] = 1.0 if str(get_val('Customer Type')).strip() == 'disloyal Customer' else 0.0
        ...
        engineered['last_login_days_log'] = np.log1p(float(pd.to_numeric(get_val('Last_Login_Days'), errors='coerce') or 5.0))
        engineered['feature_intensity_log'] = np.log1p(feats)
        engineered['support_intensity_log'] = np.log1p(float(pd.to_numeric(get_val('support_tickets'), errors='coerce') or 0.0))
        engineered['session_strength_log'] = np.log1p(time / (logins + 1.0))
        ...
        engineered['loyalty_shock_score'] = (1.0 - (nps/10.0)) * (1 / (tenure + 1))
        engineered['loyalty_resilience'] = tenure * csat
        engineered['is_zombie_user'] = 1.0 if logins < 2.0 else 0.0
        engineered['engagement_efficiency'] = time / (feats + 1.0)
    ...
    # 3. Final Casting: Strings for Cats, Floats for others
    required = PREPROCESSORS[sector].feature_names_in_
    final_dict = {}
    for col in required:
        if col in CAT_COLS:
            val = get_val(col, "unknown")
            final_dict[col] = str(val) if val is not None else "unknown"
        elif col in engineered:
            final_dict[col] = float(engineered[col])
        else:
            final_dict[col] = float(pd.to_numeric(get_val(col), errors='coerce') or 0.0)

    final_df = pd.DataFrame([final_dict])
    for col in required:
        if col in CAT_COLS: final_df[col] = final_df[col].astype(str)
...
def predict(payload: PredictPayload, api_key: str = Security(get_api_key)):
    ...
    return {
        "prediction_id": str(uuid.uuid4()), "probability": round(float(prob), 4),
        "trigger_diagnosis": top_driver, "status": "success",
